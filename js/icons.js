// js/icons.js — Set de iconos SVG inline (estilo Lucide, MIT) para reemplazar
...
// así combinan con la paleta De Cirene sin tener que pasar color en cada llamada.

// Cada entrada es el contenido interno de un <svg viewBox="0 0 24 24">.
// Trazos (stroke) — coherentes, limpios, profesionales.
const PATHS = {
  ...
  gantt: '<path d="M8 6h10"/><path d="M6 12h9"/><path d="M11 18h7"/><path d="M3 3v18"/>',
...
// Aliases temáticos (un mismo path para varios usos)
const ALIAS = {
  stock: 'package',
  catalog: 'package',
  invoices: 'file',
...
export function icon(name, { size = 18, color = 'currentColor', stroke = 2, cls = '' } = {}) {
  const key = ALIAS[name] || name;
  const body = PATHS[key];
  if (!body) { console.warn('[icons] icono no encontrado:', name); return ''; }
  return `<svg class="gl-icon ${cls}" xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" `
    + `viewBox="0 0 24 24" fill="none" stroke="${color}" stroke-width="${stroke}" `
    + `stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" `
    + `style="vertical-align:middle;flex:0 0 auto">${body}</svg>`;
}
